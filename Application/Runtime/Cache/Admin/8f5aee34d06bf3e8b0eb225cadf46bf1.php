...
<body>
    ...
    <div class="content-box">
        <div class="main_option">
            <ul>
                <li id="add">新增</li>
                <li class="batch" data-href="<?php echo U('delvillage');?>">删除</li>
                <?php if(($back) == "1"): ?><li class="back" onclick="javascript:;window.history.back()">返回</li><?php endif; ?>
            </ul>
            <div class="search">
                <input type="text" name="name" class="search-input" value="<?php echo I('name');?>" placeholder="请输入小区名称">
                <button id="search" data-href="<?php echo U('village');?>" data-name="name">搜索</button>
            </div>
        </div>
        <div class="content_list">
            <table>
                ...
                <tbody>
                    <?php if(!empty($villagelists)): if(is_array($villagelists)): $i = 0; $__LIST__ = $villagelists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($i%2 == 0): ?><tr>
                                    <?php else: ?>
                            ...
                            <td>
                                <span class="edit_option" data-href="<?php echo U('editvillage?id='.$vo['id']);?>" data-title="编辑菜单">编辑</span>
                                <a href="/index.php/Admin/System/delvillage/id/<?php echo ($vo["id"]); ?>" class="confirm ajax-get">删除</a>
                            </td>
