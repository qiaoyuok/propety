...
<body>
    ...
    <div class="sidebar">
            ...
            <?php if(!empty($menus)): if(is_array($menus)): $i = 0; $__LIST__ = $menus;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(is_array($vo)): $index = 0; $__LIST__ = $vo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($index % 2 );++$index; if(($index) == "1"): if(!empty($v["group"])): ?><p><?php echo ($v["group"]); ?></p>
                            ...
                            <li><a class="menu<?php echo ($v["option"]); ?>" href="/index.php/Admin/Complaint/../<?php echo ($v["url"]); ?>.html"><?php echo ($v["title"]); ?></a></li>
                            <?php else: ?>
                            <li><a class="menu<?php echo ($v["option"]); ?>" href="/index.php/Admin/Complaint/../<?php echo ($v["url"]); ?>.html"><?php echo ($v["title"]); ?></a></li><?php endif; endforeach; endif; else: echo "" ;endif; endforeach; endif; else: echo "" ;endif; endif; ?>
    ...
    <div class="main_option">
        <ul>
            <li class="batch" data-href="<?php echo U('dealed');?>">确认处理</li>
            <li class="batch" data-href="<?php echo U('dealing');?>">恢复处理</li>
            <li class="batch" data-href="<?php echo U('delete');?>">删除</li>
            <?php if(($back) == "1"): ?><li class="back" onclick="javascript:;window.history.back()">返回</li><?php endif; ?>
        </ul>
        <div class="search">
            <input type="text" name="username" class="search-input" value="<?php echo I('username');?>" placeholder="请输入用户姓名">
            <button id="search" data-href="<?php echo U('index');?>" data-name="username">搜索</button>
        </div>
    </div>
    <div class="content_list">
        <table>
            ...
            <tbody>
                <?php if(!empty($complaintlists)): if(is_array($complaintlists)): $i = 0; $__LIST__ = $complaintlists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($i%2 == 0): ?><tr>
                                <?php else: ?>
                        ...
                        <td><?php if(($vo["status"]) == "1"): ?><b style="color: green;">已处理</b><?php else: ?><b style="color: red;">待处理</b><?php endif; ?></td>
                        <td class="view_content" data-content="<?php echo ($vo["content"]); ?>" data-title="投诉内容"><?php echo (strdeal($vo["content"])); ?></td>
                        <td><?php echo ($vo["create_time"]); ?></td>
                        <td><?php echo ($vo["update_time"]); ?></td>
                        <td>
                            <?php if(($vo["status"]) == "0"): ?><a href="/index.php/Admin/Complaint/dealed/id/<?php echo ($vo["id"]); ?>" class="confirm ajax-get">确认处理</a>
                                <?php else: ?>
                                <a href="/index.php/Admin/Complaint/dealing/id/<?php echo ($vo["id"]); ?>" class="confirm ajax-get">恢复处理</a><?php endif; ?>
                            <!-- <span class="edit_option" data-href="<?php echo U('adminedit?uid='.$vo['uid']);?>" data-title="编辑管理员">编辑</span> -->
                            <a href="/index.php/Admin/Complaint/delete/id/<?php echo ($vo["id"]); ?>" class="confirm ajax-get">删除</a>
                        </td>
    ...
    $(".complaint_nav").attr("class", "active")
...
$("#login_out").click(function() {
    ...
    $.post("/index.php/Admin/Complaint/../Login/loginout", "", function(res) {
        location.reload();
